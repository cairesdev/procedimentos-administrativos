...
const API = process.env.API_URL ?? "http://localhost:8004";
...
const chamar = async (
  ...
  if (!resposta.ok) {
    throw new Error(
      `${metodo} ${rota} → ${resposta.status} ${JSON.stringify(dados)}`,
    );
  }
  return dados;
};
...
const logar = async (identificador: string) => {
  const { token } = await chamar("POST", "/auth/login", {
    identificador,
    senha: SENHA,
  });
...
const executar = async () => {
  ...
  const unidade = await chamar(
    "POST",
    "/unidades",
    { nome: `Secretaria de Saúde ${marca}`, sigla: "SMS" },
    admin,
  );
  const protocolo = await chamar(
    "POST",
    "/setores",
    { nome: `Protocolo ${marca}`, tipo: "PROTOCOLO" },
    admin,
  );
  const compras = await chamar(
    "POST",
    "/setores",
    { nome: `Compras ${marca}`, tipo: "COMPRAS" },
    admin,
  );
  const controladoria = await chamar(
    "POST",
    "/setores",
    { nome: `Controladoria ${marca}`, tipo: "CONTROLADORIA" },
    admin,
  );
  ...
  await chamar(
    "PUT",
    "/fluxos/SOLICITACAO_ITENS",
    {
      permiteOverrideUsuario: false,
      etapas: [
        {
          ordem: 1,
          setorId: protocolo.id,
          prazoDias: 5,
          prazoAtivo: true,
        ...
        {
          ordem: 3,
          setorId: controladoria.id,
          prazoAtivo: false,
          visibilidadeEstendida: true,
        },
      ],
    },
    admin,
  ...
  const uProtocolo = await criarUsuario(admin, "PROTOCOLO", {
    setorId: protocolo.id,
  });
  const uCompras = await criarUsuario(admin, "COMPRAS", {
    setorId: compras.id,
  });
  const uControle = await criarUsuario(admin, "CONTROLADORIA", {
    setorId: controladoria.id,
  });
  const uServidor = await criarUsuario(admin, "SERVIDOR", {
  ...
  conferir(
    duplicada.status === 409,
    "número de licitação duplicado devolve 409",
  );

  const contrato = await chamar(
    "POST",
    "/contratos",
    {
      numero: `CT-${marca}`,
  ...
  conferir(
    copo.saldoDisponivel === 100,
    "saldo inicial do item = quantidade total",
  );
  ...
  conferir(
    semReserva.find((i: any) => i.id === copo.id).saldoDisponivel === 100,
    "rascunho não reserva saldo",
  );

  const detalhe = await chamar(
    "GET",
    `/solicitacoes/${rascunho.id}`,
    undefined,
    tokenServidor,
  );
  const valorCopo = detalhe.itens.find(
    (i: any) => i.itemId === copo.id,
  ).valorCalculado;
  const valorServico = detalhe.itens.find(
    (i: any) => i.itemId === servico.id,
  ).valorCalculado;
  conferir(valorCopo === 20, "cálculo UNIDADE: 20 × R$1,00 = R$20,00");
  conferir(
    valorServico === 500,
    "cálculo PERCENTUAL: 10% de R$5.000,00 = R$500,00",
  );

  const envio = await chamar(
    "POST",
    `/solicitacoes/${rascunho.id}/enviar`,
    {},
    tokenServidor,
  );
  conferir(
    !!envio.protocolo,
    `envio gera protocolo ${envio.protocolo} e processo ${envio.processoAdm}`,
  );

  const comReserva = await chamar(
    "GET",
    `/contratos/${contrato.id}/itens`,
    undefined,
    admin,
  ...
  conferir(
    comReserva.find((i: any) => i.id === copo.id).saldoDisponivel === 80,
    "envio reserva saldo: 100 − 20 = 80",
  );

  const processoId = envio.processoId;
  const processo = await chamar(
    "GET",
  ...
  conferir(
    processo.setorAtualId === protocolo.id,
    "processo cai na 1ª etapa do fluxo (Protocolo)",
  );
  ...
  await chamar(
    "POST",
    `/processos/${processoId}/despachos`,
    {
      lotacaoId: uProtocolo.lotacaoId,
      tipo: "ANALISE",
      texto: "Processo autuado e conferido.",
    },
    tokenProtocolo,
  );
  await chamar(
    ...
    `/processos/${processoId}/despachos`,
    {
      lotacaoId: uProtocolo.lotacaoId,
      tipo: "ENCAMINHAMENTO",
      texto: "Segue para Compras.",
    },
    tokenProtocolo,
  );
  const emCompras = await chamar(
    "GET",
    `/processos/${processoId}`,
    undefined,
    tokenProtocolo,
  ...
  conferir(
    emCompras.setorAtualId === compras.id,
    "encaminhamento sem destino segue o fluxo → Compras",
  );

  const tokenCompras = await logar(uCompras.username);
  const ordem = await chamar(
    "POST",
    `/processos/${processoId}/ordens`,
    {
  ...
  conferir(
    /^\d{4}\/\d{4}$/.test(ordem.numero),
    `ordem de fornecimento emitida (${ordem.numero})`,
  );
  ...
  const nfRepetida = await fetch(`${API}/processos/${processoId}/ordens`, {
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${tokenCompras}`,
    },
    body: JSON.stringify({
      lotacaoId: uCompras.lotacaoId,
      contratoId: contrato.id,
  ...
  conferir(
    nfRepetida.status === 409,
    "NF duplicada para o mesmo fornecedor devolve 409",
  );

  const parecerPorCompras = await fetch(
    `${API}/processos/${processoId}/parecer`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  await chamar(
    ...
    `/processos/${processoId}/despachos`,
    {
      lotacaoId: uCompras.lotacaoId,
      tipo: "ENCAMINHAMENTO",
      texto: "Instruído, segue para parecer.",
    },
    tokenCompras,
  );
  ...
  const lotacaoAlheia = await fetch(`${API}/processos/${processoId}/parecer`, {
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${tokenControle}`,
    },
    body: JSON.stringify({ lotacaoId: uCompras.lotacaoId, favoravel: true }),
  });
  conferir(
    lotacaoAlheia.status === 403,
    "lotação de outro usuário é recusada (403)",
  );

  await chamar(
    "POST",
    `/processos/${processoId}/parecer`,
    {
      lotacaoId: uControle.lotacaoId,
      favoravel: true,
      justificativa: "Regularidade confirmada.",
    },
    tokenControle,
  );
  const encerrado = await chamar(
    "GET",
    `/processos/${processoId}`,
    undefined,
    tokenControle,
  ...
  conferir(
    ...
    "parecer favorável encerra o processo",
  );
  conferir(
    encerrado.despachos.length === 5,
    `timeline com ${encerrado.despachos.length} despachos`,
  );

  const saldoFinal = await chamar(
    "GET",
    `/contratos/${contrato.id}/itens`,
    undefined,
    admin,
  ...
  conferir(
    saldoFinal.find((i: any) => i.id === copo.id).saldoDisponivel === 80,
    "aprovação mantém o saldo debitado",
  );

  // --- Cenário 2: cancelamento devolve saldo ------------------------------
  const outra = await chamar(
    "POST",
    "/solicitacoes",
    {
      unidadeSolicitanteId: unidade.id,
  ...
  conferir(
    devolvido.find((i: any) => i.id === copo.id).saldoDisponivel === 80,
